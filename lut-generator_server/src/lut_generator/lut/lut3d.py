...
class LUT3DConfig:
    ...
    # ``smoothness`` is a legacy kwarg the integration tests pass in but the
    # current generator does not consume (the underlying interpolation is
    # driven by ``interpolation``).  Accept and store it so callers that
    # built config objects against the older API keep working.
    smoothness: float = 0.5
    # Same story for ``use_advanced_interpolation``: kept for backwards
    # compatibility with callers that pre-date the ``interpolation`` field.
    use_advanced_interpolation: bool = False
...
class LUT3DGenerator:
    ...
    def apply_trilinear_interpolation(self, inputs: np.ndarray) -> np.ndarray:
        """
        应用三线性插值到输入 RGB 点。

        Args:
            inputs: ``(N, 3)`` 或 ``(3,)`` RGB 点,值 ∈ [0, 1]

        Returns:
            插值后的 RGB,shape 与输入一致(``(N, 3)`` 或 ``(3,)``)
        """
        if self.lut_data is None:
            raise ValueError("LUT data not generated. Call generate_* first.")
        ...
        result = self._interpolator.interpolate(
            self.lut_data, inputs.astype(np.float32), self.config.grid_size
        )
        if single:
...
def generate_lut_3d(source_stats: ColorStatistics,
    ...
    Args:
        ...
        grid_size: LUT 精度
        strength: 迁移强度
        use_vectorized: 是否使用向量化生成(默认 True,性能高;False 用于调试/低内存)
